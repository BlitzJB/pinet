  ...
  return (
    <div className={cn(paper, "flex w-full flex-col gap-1 rounded-[24px] p-2.5 shadow-lg shadow-black/5 transition-colors")}>
      ...
      <div className="flex items-center gap-1 px-1">
        {model && (
          <span className={cn(mono, "truncate rounded-full bg-foreground/[0.05] px-2 py-1 text-foreground/45")} title={`${model.provider}/${model.id}`}>
            {model.id}
          </span>
        )}
        ...
        <span className="ml-auto hidden pr-1 text-[11px] text-foreground/25 sm:block">Enter to send</span>
        <button
          type="button"
